...
export function createOnboardingController(deps: {
    ...
    return {
        ...
        async handleSetup(req: Request, res: Response): Promise<void> {
            ...
            if (!attempt) {
                res.status(400).send(
                    '<h3>Connection request expired or already consumed. Please run /gh connect again.</h3>'
                );
                return;
            }
...
const ADMIN_SUBCOMMANDS = new Set(['connect', 'disconnect', 'repo.watch', 'repo.unwatch']);
...
export function decorateResponse(
    ...
    if (payload.embeds && payload.embeds.length > 0) {
        const modifiedEmbeds = payload.embeds.map((embed) => {
            const plain =
                'toJSON' in embed && typeof (embed as any).toJSON === 'function'
                    ? (embed as any).toJSON()
                    : { ...embed };
            return {
                ...plain,
...
describe('Pipeline - MultiTenant Routing', () => {
    it('should fail closed if guild connection is disconnected or installation is suspended', async () => {
        ...
        const matched = await routeEventToSubscriptions({ repositoryId: 42, installationId: 1001 } as any, {
